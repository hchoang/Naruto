 ...
 * @author satthuvdh
 */
public class SoundNinja1 implements Enemy {
    ...
    public SoundNinja1() {
        try {
            img = Image.createImage("/enemy1.gif");
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        ai = new EnemyAI();
        sprite = new Sprite(img, 43, 61);
        setPosition(new Position(240, 225));
        alive = true;
        maxHP = 100;
        currentHP = 100;
        score = 75;
...
//        }
    ...
    public void hit() {
        try {
            img = Image.createImage("/enemy1hit.gif");
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        sprite.setImage(img, 67, 58);
        setState(Ninja.HIT);
